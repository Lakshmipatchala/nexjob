...
export default function SignupPage() {
  ...
  async function handleSignup(e: React.FormEvent) {
    ...
    if (data.user) {
      ...
      await supabase.from("profiles").insert({ id: data.user.id, email, first_name: parts[0] || "", last_name: parts.slice(1).join(" ") || "" })
  ...
  return (
    <div style={{width:"100%",maxWidth:"420px",background:"hsl(224 71% 6%)",border:"1px solid hsl(216 34% 17%)",borderRadius:"16px",padding:"40px"}}>
      ...
      <form onSubmit={handleSignup}>
        ...
        <button type="submit" disabled={loading} style={{width:"100%",background:"#7c3aed",color:"white",padding:"12px",borderRadius:"10px",fontSize:"14px",fontWeight:"600",border:"none",cursor:"pointer",opacity:loading?0.7:1}}>{loading?"Creating account...":"Create account"}</button>
      </form>
      <p style={{textAlign:"center",marginTop:"24px",fontSize:"14px",color:"hsl(215 20% 65%)"}}>Already have an account? <Link href="/login" style={{color:"#a78bfa",fontWeight:"600",textDecoration:"none"}}>Sign in</Link></p>
    </div>
  )
}