...
export default function RecommendationsPage() {
  ...
  function scoreColor(score: number) {
    if (score >= 80) return { color: "#4ade80", bg: "rgba(34,197,94,0.15)", label: "Excellent match" }
    if (score >= 60) return { color: "#fbbf24", bg: "rgba(251,191,36,0.15)", label: "Good match" }
    return { color: "#f87171", bg: "rgba(248,113,113,0.15)", label: "Partial match" }
  }
  ...
  if (!profile?.resume_summary && !profile?.desired_title) {
    return (
      <div style={{ minHeight: "100vh", background: "hsl(224 71% 4%)", padding: "32px", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ textAlign: "center" as const, maxWidth: "480px" }}>
          <div style={{ fontSize: "64px", marginBottom: "24px" }}>🎯</div>
          <h1 style={{ fontSize: "24px", fontWeight: "700", color: "hsl(213 31% 91%)", marginBottom: "12px" }}>Set up your profile first</h1>
          <p style={{ color: "hsl(215 20% 65%)", fontSize: "15px", marginBottom: "28px", lineHeight: "1.6" }}>
            To get personalized job recommendations, we need to know about your experience and what you are looking for.
          </p>
  ...
  return (
    <div style={{ minHeight: "100vh", background: "hsl(224 71% 4%)", padding: "32px" }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "28px", flexWrap: "wrap" as const, gap: "16px" }}>
        <div>
          <h1 style={{ fontSize: "28px", fontWeight: "700", color: "hsl(213 31% 91%)", margin: "0 0 6px" }}>
            🎯 Recommended for You
          </h1>
      ...
      {profile && (
        <div style={{ background: "hsl(224 71% 6%)", border: "1px solid rgba(124,58,237,0.3)", borderRadius: "12px", padding: "20px", marginBottom: "24px", display: "flex", gap: "24px", flexWrap: "wrap" as const }}>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: "12px", color: "hsl(215 20% 45%)", marginBottom: "4px" }}>LOOKING FOR</div>
            <div style={{ fontSize: "15px", fontWeight: "600", color: "hsl(213 31% 91%)" }}>{profile.desired_title || "Not set"}</div>
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: "12px", color: "hsl(215 20% 45%)", marginBottom: "4px" }}>EXPERIENCE</div>
            <div style={{ fontSize: "15px", fontWeight: "600", color: "hsl(213 31% 91%)" }}>{profile.years_experience ? `${profile.years_experience} years` : "Not set"}</div>
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: "12px", color: "hsl(215 20% 45%)", marginBottom: "4px" }}>LOCATION</div>
            <div style={{ fontSize: "15px", fontWeight: "600", color: "hsl(213 31% 91%)" }}>{profile.desired_location || "Not set"}</div>
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: "12px", color: "hsl(215 20% 45%)", marginBottom: "4px" }}>TARGET SALARY</div>
            <div style={{ fontSize: "15px", fontWeight: "600", color: "hsl(213 31% 91%)" }}>{profile.target_salary || "Not set"}</div>
          </div>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div style={{ textAlign: "center" as const, padding: "64px", color: "hsl(215 20% 45%)" }}>
          <div style={{ fontSize: "48px", marginBottom: "16px" }}>🔍</div>
          <p style={{ fontSize: "16px", marginBottom: "8px" }}>No recommendations yet</p>
        ...
        <div style={{ display: "flex", flexDirection: "column" as const, gap: "16px" }}>
          {jobs.map((job, index) => {
            ...
            return (
              <div key={job.id} style={{ background: "hsl(224 71% 6%)", border: "1px solid hsl(216 34% 17%)", borderRadius: "14px", padding: "24px", position: "relative" as const }}>
                {/* Rank badge */}
                <div style={{ position: "absolute" as const, top: "20px", left: "-12px", background: index < 3 ? "linear-gradient(135deg, #7c3aed, #a855f7)" : "hsl(224 71% 10%)", color: "white", width: "28px", height: "28px", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "12px", fontWeight: "700", border: "2px solid hsl(224 71% 4%)" }}>
                  {index + 1}
                </div>
                ...
                <div style={{ display: "flex", alignItems: "flex-start", gap: "16px", flexWrap: "wrap" as const }}>
                  ...
                  <div style={{ flexShrink: 0 }}>
                    {job.company_logo ? (
                      <img src={job.company_logo} alt={job.company} style={{ width: "48px", height: "48px", borderRadius: "10px", objectFit: "contain", background: "white", padding: "4px" }} />
                    ) : (
                      <div style={{ width: "48px", height: "48px", borderRadius: "10px", background: "linear-gradient(135deg, #7c3aed, #a855f7)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "18px", fontWeight: "700", color: "white" }}>
                  ...
                  <div style={{ flex: 1, minWidth: "200px" }}>
                    <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "12px", flexWrap: "wrap" as const, marginBottom: "6px" }}>
                      <div>
                        <div style={{ fontSize: "17px", fontWeight: "700", color: "hsl(213 31% 91%)", marginBottom: "3px" }}>{job.title}</div>
                        <div style={{ fontSize: "14px", color: "hsl(215 20% 65%)" }}>{job.company} · {job.location}</div>
                      </div>
                      {/* Match score */}
                      <div style={{ background: sc.bg, border: `1px solid ${sc.color}30`, borderRadius: "10px", padding: "8px 14px", textAlign: "center" as const, flexShrink: 0 }}>
                        <div style={{ fontSize: "22px", fontWeight: "800", color: sc.color }}>{job.match_score}%</div>
                        <div style={{ fontSize: "11px", color: sc.color, fontWeight: "600" }}>{sc.label}</div>
                      </div>
                    </div>
                    ...
                    {job.match_reasons && job.match_reasons.length > 0 && (
                      <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" as const, marginBottom: "14px" }}>
                        {job.match_reasons.map((reason, i) => (
                          <span key={i} style={{ fontSize: "12px", padding: "3px 10px", borderRadius: "20px", background: "rgba(124,58,237,0.1)", color: "#a78bfa", border: "1px solid rgba(124,58,237,0.2)" }}>
                            ✓ {reason}
                          </span>
                        ))}
                      </div>
                    ...
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "8px", flexWrap: "wrap" as const }}>
                      ...
                      <div style={{ display: "flex", gap: "8px" }}>
                        <button onClick={() => toggleSave(job)} disabled={isSaving}
                          style={{ background: saved ? "rgba(124,58,237,0.2)" : "transparent", border: saved ? "1px solid rgba(124,58,237,0.4)" : "1px solid hsl(216 34% 25%)", borderRadius: "8px", padding: "8px 14px", cursor: "pointer", fontSize: "13px", color: saved ? "#a78bfa" : "hsl(215 20% 55%)", fontWeight: "600" as const }}>
                          {isSaving ? "..." : saved ? "★ Saved" : "☆ Save"}
                        </button>
                        <button onClick={() => sendToResume(job)}
                          style={{ background: "hsl(224 71% 10%)", color: "#a78bfa", padding: "8px 14px", borderRadius: "8px", fontSize: "13px", fontWeight: "600", border: "1px solid rgba(124,58,237,0.3)", cursor: "pointer" }}>
                          AI Resume
                        </button>
                        <button onClick={() => applyAndTrack(job)}
                          style={{ background: "#7c3aed", color: "white", padding: "8px 20px", borderRadius: "8px", fontSize: "13px", fontWeight: "600", border: "none", cursor: "pointer" }}>
                          Apply →
                        </button>
                      </div>
                    </div>
