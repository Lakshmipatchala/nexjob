...
export default function SavedPage() {
  ...
  async function loadSaved() {
    ...
    const { data } = await supabase.from("saved_jobs").select("*").eq("user_id", user.id).order("saved_at", { ascending: false })
    setSaved(data || [])
    setLoading(false)
  }
    ...
    <div style={{ minHeight: "100vh", background: "hsl(224 71% 4%)", padding: "32px" }}>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: "center", padding: "64px", color: "hsl(215 20% 45%)" }}>
          ...
          <p style={{ fontSize: "13px" }}>Tap the bookmark icon on any job card to save it here</p>
        </div>
      ) : (
            ...
            return (
              <div key={row.id} style={{ background: "hsl(224 71% 6%)", border: "1px solid hsl(216 34% 17%)", borderRadius: "12px", padding: "20px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "12px" }}>
                  ...
                  <button onClick={() => unsave(row.id)}
                    style={{ background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.2)", borderRadius: "6px", padding: "4px 8px", cursor: "pointer", fontSize: "12px", color: "#f87171" }}>
                    Remove
                  </button>
                </div>
                <div style={{ marginBottom: "12px" }}>
                  {job.work_mode && <span style={badge(job.work_mode, "#a78bfa", "rgba(124,110,245,0.15)")}>{job.work_mode}</span>}
                  {job.job_type && <span style={badge(job.job_type, "#60a5fa", "rgba(96,165,250,0.1)")}>{job.job_type}</span>}
